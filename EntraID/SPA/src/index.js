...
if (!storedLoginRequest) {
    localStorage.setItem('entraIDLoginRequest', JSON.stringify(loginRequest));
}

let storedMsalConfig = localStorage.getItem('entraIDMsalConfig');
if (!storedMsalConfig) {
    storedMsalConfig = JSON.stringify(msalConfig);
    localStorage.setItem('entraIDMsalConfig', storedMsalConfig);
}
 ...
 */
const msalInstance = new PublicClientApplication(entraIDMsalConfig);
