...
const TokenList = ({ }) => {
    ...
    useEffect(() => {
        const handleAcquireToken = () =>  {
            ...
            //instance.acquireTokenSilent(b2cloginRequest)
                    ...
                    <tbody>
                        {tokens.map((token, index) => (
                            <tr key={index} className={newToken && index === 0 ? 'new-data' : ''}>
                                ...
                                {/* <td>
                                    {token.idToken} ? 
                                    {token.idToken.slice(0, 35)}... <a href={`https://jwt.ms/#id_token=${token.idToken}`} target="_blank" rel="noopener noreferrer">View & Decode</a>
                                    : ''
                                </td>
                                <td>
                                    {token.accessToken} ?
                                    {token.accessToken.slice(0, 35)}... <a href={`https://jwt.ms/#access_token=${token.accessToken}`} target="_blank" rel="noopener noreferrer">View & Decode</a>
                                    : ''
                                </td> */}
