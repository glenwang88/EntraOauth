...
export const Configurations = () => {
    ...
    const [signUpSignInPolicy, setSignUpSignInPolicy] = useState('');
    const [editProfilePolicy, setEditProfilePolicy] = useState('');
    const [forgotPasswordPolicy, setForgotPasswordPolicy] = useState('');
    ...
    useEffect(() => {
        const storeB2cPolicies = localStorage.getItem('b2cPolicies');
        if (storeB2cPolicies) {
            const {
                names: {
                ...
                /* authorities: {
                    signUpSignIn: {
                        //authority: signUpignInPolicy
                    }
                },
                authorityDomain, */
                tenantName
            } 
            = JSON.parse(storeB2cPolicies);

            //const { auth: { clientId, authority }, cache: { cacheLocation } }
            //    = JSON.parse(storeB2cPolicies);
            setTenantName(tenantName || '');
            setSignUpSignInPolicy(signUpSignInPolicy || '');
            setEditProfilePolicy(editProfilePolicy || '');
            setForgotPasswordPolicy(forgotPasswordPolicy || '');
        }

        const storedMsalConfig = localStorage.getItem('b2cMsalConfig');
        if (storedMsalConfig) {
            const { auth: { clientId }, cache: { cacheLocation } }
    ...
    const handleSave = (event) => {
        try {
            ...
            const tenantDomain = `${tenantName}.b2clogin.com`;
            const signupSignInAuthority = `https://${tenantName}.b2clogin.com/${tenantName}.onmicrosoft.com/${signUpSignInPolicy}`;
            const forgotPasswordAuthority = `https://${tenantName}.b2clogin.com/${tenantName}.onmicrosoft.com/${forgotPasswordPolicy}`;
            const editProfileAuthority = `https://${tenantName}.b2clogin.com/${tenantName}.onmicrosoft.com/${editProfilePolicy}`;

            localStorage.setItem('b2cPolicies', JSON.stringify({
                names: {
                    signUpSignIn: signUpSignInPolicy,
                    forgotPassword: forgotPasswordPolicy,
                    editProfile: editProfilePolicy,
                ...
                authorityDomain: tenantDomain,
                tenantName: tenantName
            }));
            localStorage.setItem('b2cMsalConfig', JSON.stringify({
                auth: {
                    clientId: clientId,
                    //authority: authority
                    authority: signupSignInAuthority,
            ...
            localStorage.setItem('b2cLoginRequest', JSON.stringify({
                    scopes: scopes.split(' '),
    ...
    return (
            ...
            <Card className="bg-light" style={{ marginTop: '40px', marginBottom: '40px'  }}>
                ...
                <Card.Header> OAuth Configuration Form - B2C</Card.Header>
                <Card.Body>
                    <Form onSubmit={handleSave}>
                        <Form.Group as={Row} className="mb-3">
                            <Form.Label column sm="2">Tenant Name: <span style={{ color: 'red' }}>*</span> </Form.Label>
                            <Col sm="10">
                                <Form.Control type="text" value={tenantName} onChange={(e) => setTenantName(e.target.value)} required />
                            </Col>
                        </Form.Group>
                        <Form.Group as={Row} className="mb-3">
                        ...
                        <Form.Group as={Row} className="mb-3">
                            <Form.Label column sm="2">SignUpSignIn Policy: <span style={{ color: 'red' }}>*</span> </Form.Label>
                            <Col sm="10">
                                <Form.Control type="text" value={signUpSignInPolicy} onChange={(e) => setSignUpSignInPolicy(e.target.value)} required />
                            </Col>
                        </Form.Group>
                        <Form.Group as={Row} className="mb-3">
                            <Form.Label column sm="2">EditProfile Policy: </Form.Label>
                            <Col sm="10">
                                <Form.Control type="text" value={editProfilePolicy} onChange={(e) => setEditProfilePolicy(e.target.value)} />
                            </Col>
                        </Form.Group>
                        <Form.Group as={Row} className="mb-3">
                            <Form.Label column sm="2">ForgotPassword Policy: </Form.Label>
                            <Col sm="10">
                                <Form.Control type="text" value={forgotPasswordPolicy} onChange={(e) => setForgotPasswordPolicy(e.target.value)} />
                            </Col>
                        </Form.Group>
                        
                        {/* <Form.Group as={Row} className="mb-3">
                            <Form.Label column sm="2">Authority: <span style={{ color: 'red' }}>*</span> </Form.Label>
                            <Col sm="10">
                                <Form.Control type="text" value={authority} onChange={(e) => setAuthority(e.target.value)} />
                            </Col>
                        </Form.Group> */}
                        <Form.Group as={Row} className="mb-3">
